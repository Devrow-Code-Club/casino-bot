...
${this.betTypes
  .map(
    type =>
      `${type.name}${' '.repeat(15 - type.name.length)} - ${type.desc}${' '.repeat(
        40 - type.desc.length,
      )} - ${type.payout}`,
  )
  .join('\n')}
    ...
    return [
      {
        ...
        validate: num => num >= 0 && num <= 36,
        getNumbers: num => [num],
        payout: 35,
      },
      {
        name: 'split-horiz',
        desc: 'Any two adjoining numbers horizontal',
        validate: num => num % 3 !== 0 && num > 0,
        getNumbers: num => [num, num + 1],
        payout: 17,
      },
      {
        name: 'split-vert',
        desc: 'Any two adjoining numbers vertical',
        validate: num => num < 34 && num > 0,
        getNumbers: num => [num, num + 3],
        payout: 17,
      },
      {
        name: 'basket-low',
      ...
      {
        name: 'street',
        desc: 'any three numbers horizontal',
        validate: num => num % 3 === 1 && num > 0,
        getNumbers: num => [num, num + 1, num + 2],
        payout: 11,
      },
      {
        name: 'corner',
        desc: 'any four adjoining numbers in a block',
        validate: num => num % 3 !== 0 && num < 34 && num > 0,
        getNumbers: num => [num, num + 1, num + 3, num + 4],
        payout: 8,
      },
      {
        name: 'six-line',
        desc: 'any six numbers from two rows',
        validate: num => num % 3 === 1 && num < 34 && num > 0,
        getNumbers: num => [num, num + 1, num + 2, num + 3, num + 4, num + 5],
        payout: 5,
      },
      {
        name: 'column',
        desc: 'the numbers in the column',
      ...
      },
      {
        ...
        desc: 'red numbers',
        validate: () => true,
        getNumbers: () => [1, 3, 5, 7, 9, 12, 14, 16, 18, 19, 21, 23, 25, 27, 30, 32, 34, 36],
        payout: 1,
      },
      {
        name: 'black',
        desc: 'black numbers',
        validate: () => true,
        getNumbers: () => [2, 4, 6, 8, 10, 11, 13, 15, 17, 20, 22, 24, 26, 28, 29, 31, 33, 35],
        payout: 1,
      },
      {
        name: 'half',
        desc: 'first or second half of the numbers',
        validate: num => num < 3 && num > 0,
        getNumbers: num =>
          'x'
            .repeat(18)
            .split('')
            .map((n, i) => i + (num == 1 ? 1 : 19)),
        payout: 1,
      },
    ];
  }
  ...
  get spinTimeout() {
    ...
    for (const { amount, betNumbers, payout, player, betType } of this.bets) {
      if (betNumbers.map(number => Number(number)).includes(landing)) {
        ...
        winners[player.id].originalBet = Number(amount);
      } else {
