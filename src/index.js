...
const notifyWinners = winners => {
  return `${winners
    .map(
      winner =>
        `<@${winner.id}> wins ${
          winner.blackjack ? `${format(winner.bet * 1.5)} with a blackjack!` : format(winner.bet)
        }`,
    )
    .join('\n')}`;
};
...
bot.on('message', async ({ channel, author, mentions, content, guild }) => {
  if (author.id === bot.user.id || !/ca[sn]i[ns]o/.test(channel.name) || !content.startsWith('!'))
    return;
  const serverId = channel.guild.id;

  if (DEV_MODE && serverId !== process.env.TESTSERVER)
    return channel.send(`Sorry, we are closed right now.`);
  ...
  if (!jsonDB[serverId][author.id].totals)
    jsonDB[serverId][author.id].totals = {
      winAmount: 0,
      wins: 0,
      betAmount: 0,
      bets: 0,
      loans: 0,
      loanAmount: jsonDB[serverId][author.id].loan || 0,
      loanPaid: 0,
      highestBalance: jsonDB[serverId][author.id].balance,
      largestBet: 0,
      largestWin: 0,
      largestLoanBalance: jsonDB[serverId][author.id].loan || 0,
    };
  if (!jsonDB[serverId].houseStats)
    jsonDB[serverId].houseStats = {
      payouts: 0,
      income: 0,
      betTypes: {},
      totalBets: 0,
  ...
  if (content.startsWith('/casino help') || content.startsWith('!casino help'))
    channel.send(`To view your devrowcoin balance: !balance
...
More games coming soon!`);
  if (content.startsWith('/balance') || content.startsWith('!balance')) {
    channel.send(
      `${authorMention} your balance is: ${format(
        jsonDB[serverId][author.id].balance || 0,
      )} devrowcoins`,
    );
    if (jsonDB[serverId][author.id].loan)
      channel.send(
        `And I would like to remind you ${authorMention} that you owe me ${format(
          jsonDB[serverId][author.id].loan,
        )} devrowcoins`,
      );
    return;
  }
  if (content.startsWith('/loan') || content.startsWith('!loan')) {
    if (content.includes('pay')) {
      if (!jsonDB[serverId][author.id].loan)
        return channel.send(`I don't see an outstanding balance ${authorMention}`);
      const [, rawPayment] = content.match(/pay (\d+)/);
      if (!rawPayment)
        return channel.send(
          `Ok, how much of your ${
            jsonDB[serverId][author.id].loan
          } devrowcoin loan would you like to pay?`,
        );
      const payment = Number(rawPayment);
      if (payment > jsonDB[serverId][author.id].balance)
        return channel.send(`You don't have enough to do that ${authorMention}.`);
      if (payment > jsonDB[serverId][author.id].loan)
        return channel.send(
          `Please ${authorMention}, thats too much. You only owe \`${
            jsonDB[serverId][author.id].loan
          }\` devrowcoins.`,
        );
      if (payment == jsonDB[serverId][author.id].balance)
        return channel.send(
          `So I can just give you another loan? ${authorMention} that would leave you with nothing.`,
        );
      ...
      channel.send(
        `Alright, you've paid back ${payment} devrowcoins. Your outstanding loan is \`${
          jsonDB[serverId][author.id].loan
        }\` and you have a remaning balance of \`${jsonDB[serverId][author.id].balance}\``,
      );
      fs.writeFile(DBLOC, JSON.stringify(jsonDB));
      return;
    }
    if (jsonDB[serverId][author.id].balance > 0)
      return channel.send(
        `I see you have \`${
          jsonDB[serverId][author.id].balance || 0
        }\` devrowcoins. Please use up what you have before asking for more ${authorMention}.`,
      );
...
Excellent! Here is your _loan_ of \`1000\` devrowcoins.`);
    ...
    jsonDB[serverId][author.id].totals.largestLoanBalance = Math.max(
      jsonDB[serverId][author.id].loan,
      jsonDB[serverId][author.id].totals.largestLoanBalance,
    );
...
\`\`\`
${Object.entries(jsonDB[serverId][author.id].totals)
  .map(
    ([stat, value]) =>
      `${stat}${'.'.repeat(18 - stat.length + 12 - format(value, true).length)}${format(
        value,
        true,
      )}`,
  )
...
\`\`\`
${Object.entries(jsonDB[serverId].houseStats)
  .map(([stat, value]) =>
    isNaN(value)
      ? false
      : `${stat}${'.'.repeat(18 - stat.length + 12 - format(value, true).length)}${format(
  ...
  if (
    ...
    for (const command of rouletteCommands) {
      ...
      if (command.includes(' help')) {
        messages.push(Roulette.helpText);
        continue;
      }
      if (command.includes(' stats')) {
        messages.push(`Sure ${authorMention} here are the server stats:
\`\`\`
${Object.entries(jsonDB[serverId].houseStats.betTypes)
  .map(
    ([betType, { wins, losses }]) =>
      `${betType}${'.'.repeat(10 - betType.length + 12 - format(wins, true).length)}${format(
        wins,
        true,
      )}/${format(wins + losses, true)} - ${Math.round(100 * (wins / (wins + losses)))}%`,
  )
  .join('\n')}
\`\`\`
`);
      ...
      if (games[serverId].roulette && !games[serverId].roulette.betting) {
        messages.push(`Sorry ${authorMention}, bets are closed. Please wait for the next spin.`);
        continue;
      }
      {
        const match = command.match(/bet (\d+) ([\w-]+) ?(.+)?/);
        if (!match) {
          messages.push(`Sorry ${authorMention} I'm not sure of the bet you are placing`);
          continue;
        }
        ...
        const validBet =
          Number(amount) > 0 &&
          betTypeOptions.validate(Number(bet) >= 0 && Number(bet) <= 36 ? Number(bet) : -1);
        const betNumbers = betTypeOptions.getNumbers(Number(bet) >= 0 ? Number(bet) : -1);
        if (!validBet) {
          messages.push(`Thats not a valid bet ${authorMention}.`);
          continue;
        }
        if (jsonDB[serverId][author.id].balance - amount < 0) {
          messages.push(`You don't have enough devrowcoins for that bet ${authorMention}.`);
          continue;
        }
        ...
        if (!games[serverId].roulette) {
          ...
          games[serverId].roulette.lastRouletteWheel = await channel.send(
...
\`\`\``);
            ...
            for (const [playerid, { amount, originalBet, betTypes }] of Object.entries(winners)) {
              ...
              jsonDB[serverId][playerid].totals.largestWin = Math.max(
                jsonDB[serverId][playerid].totals.largestWin,
                Number(amount),
              );
              jsonDB[serverId][playerid].balance += Number(amount) + Number(originalBet);
              jsonDB[serverId][playerid].totals.highestBalance = Math.max(
                jsonDB[serverId][playerid].totals.highestBalance,
                jsonDB[serverId][playerid].balance,
              );
            ...
            if (Object.entries(winners).length > 0)
              await channel.send(
                `${Object.entries(winners)
                  .map(
                    ([playerid, { amount, betTypes }]) =>
                      `<@${playerid}> wins ${format(amount)} with \`[${Array.from(betTypes).join(
                        ', ',
                      )}]\``,
                  )
                  .join('\n')}`,
              );
            if (Object.entries(losers).length > 0)
              await channel.send(
                `${Object.entries(losers)
                  .map(
                    ([playerid, { betTypes }]) =>
                      `<@${playerid}> loses with \`[${Array.from(betTypes).join(', ')}]\``,
                  )
                  .join('\n')}`,
              );
        ...
        if (!games[serverId].roulette.playerMessages[author.id])
          games[serverId].roulette.playerMessages[author.id] = {};

        if (betTypeOptions) {
          jsonDB[serverId][author.id].balance =
            jsonDB[serverId][author.id].balance - Number(amount);
          ...
          jsonDB[serverId][author.id].totals.largestBet = Math.max(
          ...
          if (!jsonDB[serverId].houseStats.betTypes[betType])
            jsonDB[serverId].houseStats.betTypes[betType] = { times: 0, wins: 0, losses: 0 };
          jsonDB[serverId].houseStats.betTypes[betType].times++;

          games[serverId].roulette.bets.push({
            amount: Number(amount),
            betNumbers,
            payout: betTypeOptions.payout,
            player: author,
            betType,
          });
  ...
  if (content.startsWith('!blackjack ') || content.startsWith('!bj ')) {
    ...
    if (BJ_DOWN && serverId !== process.env.TESTSERVER)
      return channel.send(`Sorry - blackjack(beta) is closed right now.`);
...
\`\`\``);
    ...
    if (
      games[serverId].blackjack &&
      games[serverId].blackjack.getPlayerById(author.id) &&
      games[serverId].blackjack.started &&
      games[serverId].blackjack.getPlayerById(author.id).cardTotal > 21
    )
      return channel.send(`Sorry ${authorMention}, you busted.`);
    if (!games[serverId].blackjack && !content.includes(' bet'))
      return channel.send(`No game in progress, start a game with a \`bet\`.`);
    if (
      games[serverId].blackjack &&
      games[serverId].blackjack.insuranceOffered &&
      !content.includes(' insurance')
    )
      return channel.send(`Lets get your option on insurance first ${authorMention}`);
    if (
      games[serverId].blackjack &&
      games[serverId].blackjack.currentPlayer &&
      games[serverId].blackjack.currentPlayer.id !== author.id
    )
      return channel.send(
        `Its not your turn ${authorMention}, I'm working with <@${games[serverId].blackjack.currentPlayer.id}>.`,
      );
    if (content.includes(' bet')) {
      const match = content.match(/bet (\d+)/);
      if (!match)
        return channel.send(`Sorry ${authorMention} I'm not sure of the bet you are placing`);
      const [, amount] = match;
      ...
      if (!games[serverId].blackjack) {
        ...
        games[serverId].blackjack.messages.starting = await channel.send(`Starting in ${
          games[serverId].blackjack.startingIn
        } seconds. Place your bets to get in the game.
...
${games[serverId].blackjack.players.map(player => `<@${player.id}>`).join('\n')}`);
        ...
        if (games[serverId].blackjack.dealerAceShowing) {
          await channel.send(
            `Would anyone like to buy insurance? (\`!blackjack insurance decline/[0 - bet/2]\`)`,
          );
          setTimeout(() => {
            games[serverId].blackjack.players.forEach(player => (player.insuranceResolved = true));
          }, 15000);
          await games[serverId].blackjack.offerInsurance();
          if (!games[serverId].blackjack.dealer.blackjack)
            await channel.send(`Dealer does not have blackjack.`);
        }
        if (games[serverId].blackjack.dealer.blackjack) {
        ...
        if (games[serverId].blackjack.getPlayerById(author.id)) {
          ...
          games[serverId].blackjack.messages.starting = await games[serverId].blackjack.messages
            .starting.edit(`Starting in ${
            games[serverId].blackjack.startingIn
          } seconds. Place your bets to get in the game.
Currently in the game:
${games[serverId].blackjack.players.map(player => `<@${player.id}>`).join('\n')}`);
        ...
        if (games[serverId].blackjack.started)
          return channel.send(`Hold on ${authorMention}, there is a game in progress.`);
        if (games[serverId].blackjack)
          await channel.send(
            `Welcome ${authorMention}, try your luck? The game starts in ${games[serverId].blackjack.startingIn} seconds`,
          );
      }
    }
    if (content.includes(' insurance')) {
      ...
      if (!insurance || Number.isNaN(insurance) || insurance < 0)
        return channel.send(
          `There seems to be a problem with your insurance of ${rawInsurance}, ${authorMention}`,
        );
      if (insurance > jsonDB[serverId][author.id].balance)
        return channel.send(
          `You don't have enough to cover insurance of ${format(insurance)}. You only have ${format(
            jsonDB[serverId][author.id].balance,
          )}, ${authorMention}`,
        );
      if (insurance > player.bet / 2)
        return channel.send(
          `That insurance is too high ${authorMention}. You can only go up to ${format(
            player.bet / 2,
          )}`,
        );
      player.insuranceResolved = true;
    ...
    if (content.includes(' stand')) {
      ...
      if (games[serverId].blackjack.currentPlayer) {
        ...
        games[serverId].blackjack.currentPlayer.lastCardDisplay = await channel.send(
          ...(await games[serverId].blackjack.currentPlayer.sendCards()),
        );
      }
    }
    ...
    if (content.includes(' split')) {
      const player = games[serverId].blackjack.getPlayerById(author.id);
      if (player.cards.length > 2 || Array.isArray(player.splitCards))
        return channel.send(`It's only on the original deal ${authorMention}.`);
      if (player.cardFaces[0] !== player.cardFaces[1])
        return channel.send(
          `You can only split if you have two of the same faced cards ${authorMention}.`,
        );
      if (jsonDB[serverId][author.id].balance < player.bet)
        return channel.send(`${authorMention}, you don't have enough devrowcoin to cover a split.`);
    ...
    if (content.includes(' double down')) {
      const player = games[serverId].blackjack.getPlayerById(author.id);
      if (!player)
        return channel.send(`Sorry ${authorMention}, it doesn't look like you are in a game.`);
      if (player.cards.length > 2)
        return channel.send(`It has to be your first move, ${authorMention}.`);
      if (jsonDB[serverId][author.id].balance < player.bet)
        return channel.send(
          `${authorMention}, you don't have enough devrowcoin to cover a double down.`,
        );
      jsonDB[serverId][author.id].balance -= player.bet;
      player.bet *= 2;
      player.dealCard(games[serverId].blackjack.getCard());
    ...
    if (games[serverId].blackjack && games[serverId].blackjack.allStand) {
      ...
      if (winners.length > 0) {
        ...
        winners.forEach(player => {
          ...
          if (Number.isNaN(winAmount))
            return channel.send(
              `Uh oh, something went wrong and I'm not sure what your bet was <@${player.id}> :clenched:`,
            );
  ...
  if (content.startsWith('!wager ')) {
    ...
    if (!wagerid)
      return channel.send(
        `Please start with a wagerid (\`!wager "{wagerid}" command ...\`) to make or interact with a wager, ${authorMention}. Learn more with \`!wager help\`.`,
      );
